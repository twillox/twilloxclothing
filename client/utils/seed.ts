...
const SEED_COUPONS = [
  { code: 'LUXURY20', type: 'percentage', value: 20, minPurchase: 100, active: true },
  { code: 'WELCOME50', type: 'fixed', value: 50, minPurchase: 200, active: true },
];
...
const SEED_BANNERS = [
  {
    ...
    subtitle: 'UNCOMPROMISING ELEGANCE FOR THE MODERN WARDROBE',
    imageUrl: 'https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&q=80&w=1600',
    link: '/catalog',
    type: 'home'
  },
  {
    id: 'ban-outerwear-promo',
    title: 'OUTERWEAR REFINED',
    subtitle: 'CASHMERE AND WOOL CLASSICS',
    imageUrl: 'https://images.unsplash.com/photo-1539533113208-f6df8cc8b543?auto=format&fit=crop&q=80&w=1200',
...
export async function seedDatabase() {
  try {
    ...
    for (const ban of SEED_BANNERS) {
      const docRef = doc(db, 'banners', ban.id);
    ...
    console.log("🎉 Database fully seeded with luxury items!");
    return { success: true, message: "Database successfully populated with luxury inventory models." };
  } catch (error: any) {
    console.error("❌ Error seeding database:", error);
    throw new Error(`Seeding operation failed: ${error.message}`);
  }
}
